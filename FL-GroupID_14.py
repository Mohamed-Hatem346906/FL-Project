...
Original file is located at
    https://colab.research.google.com/drive/1znSSDi3Sl0oWVxEOXakYhRquU2LxhbMS
"""
...
def run_fedavg_experiments():
    ...
    for nc in nc_values:
        for local_epochs in local_epochs_values:
            ...
            # Resume if checkpoint exists (simplified version)
            start_round = 0
            if os.path.exists(fedavg_CHECKPOINT_PATH):
                try:
                    checkpoint = torch.load(fedavg_CHECKPOINT_PATH)
                    print(f"Checkpoint Config: nc={checkpoint['config']['nc']}, local_epochs={checkpoint['config']['local_epochs']}")
                    print(f"Current Config: nc={nc}, local_epochs={local_epochs}")

                    if checkpoint['config']['nc'] == nc and checkpoint['config']['local_epochs'] == local_epochs:
                        model.load_state_dict(checkpoint['model_state'])
                        optimizer.load_state_dict(checkpoint['optimizer_state'])
                        start_round = checkpoint['round'] + 1
                        print(f"Resumed training from round {start_round}")
                    else:
                        print("Checkpoint config mismatch (nc or local_epochs differ). Starting fresh.")
                except Exception as e:
                    print(f"Checkpoint load error: {e}. Starting fresh")
            ...
            for r in range(start_round, config.EPOCHS_FEDAVG):
                ...
                if (r + 1) % 5 == 0 or r == config.EPOCHS_FEDAVG - 1:
                    ...
                    torch.save({
                        ...
                        'model_state': model.state_dict(),
                        'optimizer_state': optimizer.state_dict(),
                        'config': {'nc': nc, 'local_epochs': local_epochs},
                        'val_acc': val_acc
